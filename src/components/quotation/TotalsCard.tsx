...
export default function TotalsCard() {
  ...
  if (data.discountEnabled) {
    if (data.discountType === 'percentage') {
      ...
      generalDiscountLabel = `Desc. ${data.discountValue}%`;
    } else {
      generalDiscountAmount = data.discountValue;
      generalDiscountLabel = `Desc. Gral.`;
    }
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-2">
        ...
        <h2 className="font-bold text-lg text-slate-800 dark:text-slate-100">Totales</h2>
      </div>
      ...
      <div className="space-y-2 text-sm">
        ...
        {data.discountEnabled && generalDiscountAmount > 0 && (
          <div className="flex justify-between text-rose-600 dark:text-rose-400">
            <span>{generalDiscountLabel}</span>
        ...
        <div className="flex justify-between text-base font-bold text-slate-900 dark:text-slate-100 pt-3 border-t-2 border-slate-200 dark:border-slate-700">
          <span>TOTAL</span>
          <span className="text-lg">{formatCurrency(total, data.currency)}</span>
        </div>
      </div>
    </motion.div>
  );
