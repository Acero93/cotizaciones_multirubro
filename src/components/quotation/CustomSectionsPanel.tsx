    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-bold text-base sm:text-lg text-slate-800 dark:text-slate-100">Secciones Personalizadas</h2>
        </div>
        <button
          onClick={() => dispatch({ type: 'ADD_CUSTOM_SECTION' })}
          className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs sm:text-sm font-medium rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
        >
      ...
      {data.customSections.map((section) => (
        <div key={section.id} className="border border-slate-200 dark:border-slate-700 rounded-xl p-3 sm:p-4 space-y-3 bg-white/50 dark:bg-slate-800/50">
          <div className="flex items-center gap-2">
            <input
              placeholder="Título de la sección"
              value={section.title}
              onChange={(e) => dispatch({ type: 'UPDATE_CUSTOM_SECTION_TITLE', id: section.id, title: e.target.value })}
              className={`flex-1 font-medium ${inputClass}`}
            />
            <button onClick={() => dispatch({ type: 'REMOVE_CUSTOM_SECTION', id: section.id })}
          ...
          {section.fields.map((field) => (
            <div key={field.id} className="flex gap-1.5 sm:gap-2 items-center">
              <input placeholder="Etiqueta" value={field.label}
                onChange={(e) => dispatch({ type: 'UPDATE_CUSTOM_SECTION_FIELD', sectionId: section.id, fieldId: field.id, label: e.target.value, value: field.value })}
                className={`flex-1 ${inputClass}`} />
              <input placeholder="Valor" value={field.value}
                onChange={(e) => dispatch({ type: 'UPDATE_CUSTOM_SECTION_FIELD', sectionId: section.id, fieldId: field.id, label: field.label, value: e.target.value })}
                className={`flex-1 ${inputClass}`} />
              <button onClick={() => dispatch({ type: 'REMOVE_CUSTOM_SECTION_FIELD', sectionId: section.id, fieldId: field.id })}
                className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors shrink-0">
                <Trash2 className="w-3.5 h-3.5" />
              </button>
            </div>
          ))}
          <button onClick={() => dispatch({ type: 'ADD_CUSTOM_SECTION_FIELD', sectionId: section.id })}
      ...
      <div className="space-y-3 pt-2">
        <textarea placeholder="Notas adicionales..." value={data.notes}
        ...
        <textarea placeholder="Términos y condiciones..." value={data.terms}
