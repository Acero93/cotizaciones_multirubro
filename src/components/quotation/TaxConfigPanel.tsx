      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={data.tax.enabled}
              onChange={(e) => dispatch({ type: 'SET_TAX', tax: { ...data.tax, enabled: e.target.checked } })}
          ...
          {data.tax.enabled && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex gap-2">
              <input placeholder="Nombre (IVA, IGV...)" value={data.tax.name}
                onChange={(e) => dispatch({ type: 'SET_TAX', tax: { ...data.tax, name: e.target.value } })}
                className={`flex-1 ${inputClass}`} />
              <div className="relative w-20 sm:w-24">
                <input type="number" min="0" max="100" value={data.tax.rate}
                  onChange={(e) => dispatch({ type: 'SET_TAX', tax: { ...data.tax, rate: Number(e.target.value) } })}
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={data.discountEnabled}
              onChange={(e) => dispatch({ type: 'SET_DISCOUNT', enabled: e.target.checked, discountType: data.discountType, value: data.discountValue })}
              className="w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-primary focus:ring-primary" />
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Aplicar descuento general</span>
          </label>
          {data.discountEnabled && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex gap-2">
              <select value={data.discountType}
                onChange={(e) => dispatch({ type: 'SET_DISCOUNT', enabled: true, discountType: e.target.value as 'percentage' | 'amount', value: data.discountValue })}
              ...
              <div className="relative flex-1">
                {data.discountType === 'percentage' ? (
                  <>
                    <input type="number" min="0" max="100" value={data.discountValue}
                      onChange={(e) => dispatch({ type: 'SET_DISCOUNT', enabled: true, discountType: 'percentage', value: Number(e.target.value) })}
                      className={`w-full text-right ${inputClass}`} />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 text-sm">%</span>
                  </>
                ) : (
                  <input type="number" min="0" value={data.discountValue}
                    onChange={(e) => dispatch({ type: 'SET_DISCOUNT', enabled: true, discountType: 'amount', value: Number(e.target.value) })}
                    className={`w-full text-right ${inputClass}`} />
                )}
              </div>
            </motion.div>
